...
if(isset($_REQUEST["rentimine"])){
    global $yhendus;
    $kask=$yhendus->prepare("UPDATE raamatukogu SET laenutus_kuup='on', saadavus='mitte' WHERE id=?");
...
// ! + knopka tab - näitab html koodi algus
    global $yhendus;
    $kask=$yhendus->prepare("SELECT id, nimi, autor, laenu_pikkus FROM raamatukogu WHERE saadavus='on'");
    ...
    while($kask->fetch()){
        ...
        echo "<td>";
            ...
            echo "<td><a href='?rentimine=$id'><button class='button-35'> Rentida</button></a></td>"; 
            echo "</tr>";
    }
